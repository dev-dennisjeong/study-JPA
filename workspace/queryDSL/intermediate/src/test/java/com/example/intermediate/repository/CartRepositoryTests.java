...
//        장바구니에서 삭제된 항목의 상품 재고 복구
        Optional<Product> foundProduct = productRepository.findById(402L);
        Optional<Cart> foundCart = cartRepository.findById(403L);
        foundCart.ifPresent(cart -> foundProduct.ifPresent(product -> product.setProductStock(product.getProductStock() + cart.getCount().intValue())));
        foundCart.ifPresent(cart -> cartRepository.delete(cart));
    }
}
